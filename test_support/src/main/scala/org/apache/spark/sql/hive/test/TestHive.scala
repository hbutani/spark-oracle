  ...
  def this(sc: SparkContext, hiveClient: HiveClient) {
    this(
      new TestHiveSparkSession(
        HiveUtils.withHiveExternalCatalog(sc),
        hiveClient,
...
private[hive] class TestHiveSparkSession(
  ...
  private def quoteHiveFile(path: String) =
    if (Utils.isWindows) {
  ...
  if (loadTestTables) {
    ...
    val hiveQTestUtilTables: Seq[TestTable] = Seq(
      ...
      TestTable(
        "srcpart",
        () => {
          "CREATE TABLE srcpart (key INT, value STRING) PARTITIONED BY (ds STRING, hr STRING)".cmd
            .apply()
      ...
      TestTable(
        "srcpart1",
        () => {
          "CREATE TABLE srcpart1 (key INT, value STRING) PARTITIONED BY (ds STRING, hr INT)".cmd
            .apply()
