...
lazy val test_support = project.in(file("test_support"))
...
lazy val orastuff = project.in(file("orastuff"))
  .disablePlugins(AssemblyPlugin)
  .settings(commonSettings: _*)
  .settings(
    libraryDependencies ++= oracle.dependencies
  )
...
lazy val spark_extend = project.in(file("packaging/spark_extend"))
  .settings(commonSettings: _*)
  .settings(Assembly.assemblySettings: _*)
  .dependsOn(test_support % "test",common, orastuff, sql, mllib)

lazy val spark_embed = project.in(file("packaging/spark_embed"))
  .settings(commonSettings: _*)
  .settings(Assembly.assemblySettings: _*)
  .dependsOn(test_support % "test",common, orastuff, sql, mllib)