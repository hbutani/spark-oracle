...
public class OracleDBConnectionCacheUtil {
    ...
    private static final String SHARDED_QUERY = "SELECT "
            + "GSMADMIN_INTERNAL.GETSHARDINGMODE FROM DUAL";
    private static final String SHARD_URL_QUERY = "SELECT connect_string "
            + "FROM sha_databases WHERE is_primary='Y'";


    /**
     * Returns whether the underlying database is sharded. If query throws an
     * exception then we proceed under the assumption that database is not sharded
     * @param conn
     * @return
     * @throws SQLException
     */
    public static boolean isShardedDB(Connection conn) throws SQLException {
        boolean isSharded = false;

        try(Statement st = conn.createStatement();
            ResultSet rs = st.executeQuery(SHARDED_QUERY);) {
            if (rs.next()) {
                final int shardMode = rs.getInt(1);
                if ((shardMode & DB_SHARD_CATALOG) != 0) {
                    isSharded = true;
                }
            }
        } catch(SQLException ex) {
            LOG.info("Oracle Database version doesn't support sharding");
        }
        return isSharded;
    }

    public static List<String> getShardedConnectionURL(Connection conn) throws
            SQLException {
        List <String> urlList = new ArrayList();
        try (Statement st = conn.createStatement();
             ResultSet rs = st
                     .executeQuery(SHARD_URL_QUERY);) {

            while (rs.next()) {
                final String connectString = rs.getString(1);
                urlList.add(connectString);
