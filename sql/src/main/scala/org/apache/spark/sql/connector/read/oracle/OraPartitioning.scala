...
case class OraUnknownDistribution(numPartitions: Int) extends Partitioning {
  override def satisfy(distribution: Distribution): Boolean = false
}

case class OraClusteredDistribution(numPartitions: Int, columns: Set[String])
    extends Partitioning {
  override def satisfy(distribution: Distribution): Boolean = distribution match {
    case cd: ClusteredDistribution => cd.clusteredColumns.forall(columns.contains(_))
    case _ => false
  }
}
