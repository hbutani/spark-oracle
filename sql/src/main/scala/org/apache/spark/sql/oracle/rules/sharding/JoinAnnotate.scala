...
object JoinAnnotate extends PredicateHelper {
  ...
  case class ShardingJoinInfo(leftKeys: Seq[Expression],
    ...
    def extractFrom(e: Expression): Option[(OpCode, ShardTable, Int)] = {
      for ((baseE, plan) <- _findExpressionAndTrackLineageDown(e, joinOp)
